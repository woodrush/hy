...
(defn hy_symbol_mangle [p] 
 ...
 (try (do (when (and ((. p startswith) "*") ((. p endswith) "*") (not_in p (, "*" "**"))) (do (setv p ((. (get p (slice 1 (- 1) None)) upper))))) 
 (when (and (in "-" p) (!= p "-")) (do (setv p ((. p replace) "-" "_")))) 
 (when (and ((. p endswith) "?") (!= p "?")) (do (setv p (% "is_%s" (get p (slice None (- 1) None)))))) 
 (when (and ((. p endswith) "!") (!= p "!")) (do (setv p (% "%s_bang" (get p (slice None (- 1) None)))))) 
 (raise (Py2HyReturnException p))) (except [e Py2HyReturnException] e.retvalue)))
(defn hy_symbol_unmangle [p] 
 "Using a hacky implementation of `return`" 
 (try (do (setv p (str_type p)) 
 (when (and ((. p endswith) "_bang") (!= p "_bang")) (do (setv p (+ (get p (slice None (- (len "_bang")) None)) "!")))) 
 (when (and ((. p startswith) "is_") (!= p "is_")) (do (setv p (+ (get p (slice (len "is_") None None)) "?")))) 
 (when (and (in "_" p) (!= p "_")) (do (setv p ((. p replace) "_" "-")))) 
 (when (and (all (list_comp (or (and ((. c isalpha)) ((. c isupper))) (= c "_")) [c p])) (any (list_comp ((. c isalpha)) [c p]))) (do (setv p (+ (+ "*" ((. p lower))) "*")))) 
 (raise (Py2HyReturnException p))) (except [e Py2HyReturnException] e.retvalue)))
(defn set_boundaries [fun] 
 ...
 (defn wrapped [p] 
 "Using a hacky implementation of `return`" 
 (try (do (setv start (. (get p 0) source_pos)) 
 (setv end (. (get p (- 1)) source_pos)) 
 (setv ret (fun p)) 
 (setv (. ret start_line) (. start lineno)) 
 (setv (. ret start_column) (. start colno)) 
 (if (is_not start end) (do (setv (. ret end_line) (. end lineno)) (setv (. ret end_column) (. end colno))) (do (setv (. ret end_line) (. start lineno)) (setv (. ret end_column) (+ (. start colno) (len (. (get p 0) value)))))) 
...
(with_decorator 
 ...
 (defn paren [p] 
 "Using a hacky implementation of `return`" 
 (try (do (setv cont (get p 1)) 
 (when (and (>= (len cont) 3) (isinstance (get cont (- 2)) HySymbol) (= (get cont (- 2)) ".")) (do (reject_spurious_dots (get cont (slice None (- 2) None)) (get cont (slice (- 1) None None))) (if (= (len cont) 3) (do (raise (Py2HyReturnException (HyCons (get cont 0) (get cont 2))))) (do (raise (Py2HyReturnException (HyCons (get cont 0) (paren [(get p 0) (get cont (slice 1 None None)) (get p 2)])))))))) 
...
(with_decorator 
 ...
 "Using a hacky implementation of `return`" 
 (try (do (setv n_stars (len (get ((. (get p 0) getstr)) (slice 1 None None)))) 
 (if (= n_stars 1) (do (setv sym "unpack_iterable")) (do (if (= n_stars 2) (do (setv sym "unpack_mapping")) (do (raise (LexException "Too many stars in `#*` construct (if you want to unpack a symbol beginning with a star, separate it with whitespace)" (. (. (get p 0) source_pos) lineno) (. (. (get p 0) source_pos) colno))))))) 
...
(with_decorator 
 ...
 (defn hash_other [p] 
 "Using a hacky implementation of `return`" 
 (try (do (setv st (get ((. (get p 0) getstr)) (slice 1 None None))) 
...
(with_decorator 
 ...
 (defn t_string [p] 
 "Using a hacky implementation of `return`" 
 (try (do (setv s (get (. (get p 0) value) (slice None (- 1) None))) 
 (do (setv _py2hy_anon_var_G_1235 ((. s split) "\"" 1)) (setv header (nth _py2hy_anon_var_G_1235 0) s (nth _py2hy_anon_var_G_1235 1))) 
...
(defn symbol_like [obj] 
 ...
 "Using a hacky implementation of `return`" 
 (try (do (try (do (raise (Py2HyReturnException (HyInteger obj)))) (except [e Py2HyReturnException] (raise e)) (except [ValueError] None)) 
 (when (in "/" obj) (do (try (do (do (setv _py2hy_anon_var_G_1236 ((. obj split) "/")) (setv lhs (nth _py2hy_anon_var_G_1236 0) rhs (nth _py2hy_anon_var_G_1236 1))) (raise (Py2HyReturnException (HyExpression [(HySymbol "fraction") (HyInteger lhs) (HyInteger rhs)])))) (except [e Py2HyReturnException] (raise e)) (except [ValueError] None)))) 
 (try (do (raise (Py2HyReturnException (HyFloat obj)))) (except [e Py2HyReturnException] (raise e)) (except [ValueError] None)) 
 (when (!= obj "j") (do (try (do (raise (Py2HyReturnException (HyComplex obj)))) (except [e Py2HyReturnException] (raise e)) (except [ValueError] None)))) 
