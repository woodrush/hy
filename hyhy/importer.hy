...
(defn get_bytecode_path [source_path] 
 "Using a hacky implementation of `return`" 
 (try (do (if PY34 (do (import [importlib.util]) (raise (Py2HyReturnException (importlib.util.cache_from_source source_path)))) (do (if (hasattr imp "cache_from_source") (do (raise (Py2HyReturnException (imp.cache_from_source source_path)))) (do (do (do (setv d (nth (os.path.split source_path) 0)) (setv f (nth (os.path.split source_path) 1)))) (raise (Py2HyReturnException (os.path.join d (re.sub "(?:\\.[^.]+)?\\Z" ".pyc" f))))))))) (except [e Py2HyReturnException] e.retvalue)))
