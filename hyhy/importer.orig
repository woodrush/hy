...
def import_buffer_to_ast(buf, module_name):
    ...
    return hyhy.compiler.hy_compile(import_buffer_to_hst(buf), module_name)


def import_file_to_ast(fpath, module_name):
    """Import content from fpath and return a Python AST."""
    return hyhy.compiler.hy_compile(import_file_to_hst(fpath), module_name)
...
def hy_eval(hytree, namespace=None, module_name=None, ast_callback=None):
    ...
    _ast, expr = hyhy.compiler.hy_compile(hytree, module_name, get_expr=True)
    ...
    return builtins.eval(ast_compile(expr, "<eval>", "eval"), namespace)
